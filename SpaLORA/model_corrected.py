...
class AttentionCorrected(nn.Module):
    ...
    def forward(self, first: torch.Tensor, second: torch.Tensor, uniform: bool = False):
        stacked = torch.stack((first, second), dim=1)
        if uniform:
            # Registered Night-3B ablation: exact constants, no softmax approximation.
            alpha = torch.full(
                (stacked.shape[0], 2), 0.5, dtype=stacked.dtype, device=stacked.device
            )
        else:
...
class EncoderOverallCorrected(nn.Module):
    def __init__(self, in1: int, out1: int, in2: int, out2: int):
        ...
        # A non-parameter forward switch; the default preserves the published
        # corrected architecture and its parameter/RNG initialization exactly.
        self.attention_mode = "learned"

    def set_attention_mode(self, mode: str):
        if mode not in ("learned", "uniform_within", "uniform_cross", "uniform_all"):
            raise ValueError("Unknown attention mode: %s" % mode)
    ...
    def forward(self, features1, features2, spatial1, feature1, spatial2, feature2):
        ...
        uniform_within = self.attention_mode in ("uniform_within", "uniform_all")
        uniform_cross = self.attention_mode in ("uniform_cross", "uniform_all")
        latent1, alpha1 = self.attention1(latent_spatial1, latent_feature1, uniform=uniform_within)
        latent2, alpha2 = self.attention2(latent_spatial2, latent_feature2, uniform=uniform_within)
        combined, alpha = self.cross_attention(latent1, latent2, uniform=uniform_cross)
