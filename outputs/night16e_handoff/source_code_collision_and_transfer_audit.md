...
The independent human-hippocampus ablation supports a narrower object: a support-weighted operator over the same-start, same-base Night-15F direct energy. `NIGHT15F_DIRECT` is 0.167510/0.266756 ARI/NMI and `SUPPORT_MODULATION_ONLY` is 0.544877/0.557795, a matched gain of +0.377368/+0.291039. The support-only control retains the inherited base self-return while disabling relation stay; the existing stay-off control disables both base and relation stay. Therefore relation-specific rejected-mass stay is not independently identified. Support weighting also changes both edge locations and total Potts capacity. Without uniform-mass-matched, permuted-support and single-view-support controls, relation-specific edge selectivity is not separated from global attenuation. `BOUNDARY_OFF` and `CONFLICT_PRIVATE_OFF` are numerically identical to full, so Night-16E makes no positive claim for the complete three-state mechanism. It also records that boundary/private relation carriers inherit the base smoothing edge, which may suppress the very relations they are intended to carry; this scientific formula is frozen and deferred rather than changed after evaluation.
