...
OUT=REPO/"outputs/night10a_rev2_handoff"; OUT.mkdir(parents=True,exist_ok=True)
    ...
    for d in DATA:
        for s in seeds_by_dataset[d]:
            for c in candidates:
                ...
                ok=tm.exists() and fm.exists() and json.loads(tm.read_text()).get("status") in {"CHECKPOINT_ROUNDTRIP_PASS","REUSED_CHECKPOINT_ROUNDTRIP_PASS"} and json.loads(fm.read_text()).get("status") in {"PASS","REUSED_PASS"}
                if ok: locked+=1
...
def evaluate(stage,candidates,seeds_by_dataset,include_r1=False):
    ...
    for d in DATA:
        ...
        labels[d]=(label_ids.tolist(),true,coords); label_audit[d]={"read_count_this_stage":1,"snapshot_sha256":digest,"keys":keys,"authorized_role":f"night10a_rev2_{stage}_evaluator","used_for_training_or_selection_before_lock":False}
    rows=[]
    for d in DATA:
