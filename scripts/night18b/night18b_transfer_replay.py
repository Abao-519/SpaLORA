#!/usr/bin/env python3
"""Fresh-process exact replay for Night-18B transfer-response candidates."""
...
from SpaLORA.night18b_transfer import TransferBackboneConfig, reload_transfer_representation
...
def run(args: argparse.Namespace) -> None:
    ...
    with np.load(args.artifact, allow_pickle=False) as artifact:
        expected_ids = artifact["ids"].astype("U"); expected_rep = artifact["representation"].astype(np.float32)
        expected_partitions = artifact["partitions"].astype(np.int32)
    if not np.array_equal(ids, expected_ids):
        raise ValueError("replay ordered-ID mismatch")
    try:
        payload = torch.load(args.checkpoint, map_location="cpu", weights_only=False)
    except TypeError:
        payload = torch.load(args.checkpoint, map_location="cpu")
    config = TransferBackboneConfig(**manifest["config"])
    with threadpool_limits(1):
        representation = reload_transfer_representation(view1, view2, retained, graphs[0], args.k, config,
                                                        manifest["arm"], payload["state_dict"], "cpu")
        partitions, _, _ = decode_embedding(representation, view1, view2, graphs, args.k)
    result = {"schema": "night18b-transfer-replay-v1", "lane": manifest["lane"], "arm": manifest["arm"],
...
def main() -> None:
    ...
    parser.add_argument("--k", type=int, required=True); parser.add_argument("--output", required=True); run(parser.parse_args())
