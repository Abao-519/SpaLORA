#!/usr/bin/env python3
"""Independent label-opening evaluator for locked Night-18B artifacts."""
...
def spatial_metrics(partition: np.ndarray, graph: sp.csr_matrix) -> dict[str, float]:
    ...
    graph.setdiag(0); graph.eliminate_zeros(); rows = np.repeat(np.arange(graph.shape[0]), np.diff(graph.indptr)); cols = graph.indices
    total_weight = max(float(graph.data.sum()), 1e-12)
    neighbour = float(np.sum(graph.data * (partition[rows] == partition[cols])) / total_weight)
    n = len(partition); weight_sum = max(float(graph.sum()), 1e-12); morans, gearys = [], []
    for cluster in np.unique(partition):
        value = (partition == cluster).astype(np.float64); centered = value - value.mean(); denom = max(float(np.sum(centered ** 2)), 1e-12)
        morans.append(float(n / weight_sum * np.sum(graph.data * centered[rows] * centered[cols]) / denom))
        gearys.append(float((n - 1) / (2 * weight_sum) * np.sum(graph.data * (value[rows] - value[cols]) ** 2) / denom))
...
def run(args: argparse.Namespace) -> None:
    with np.load(args.artifact, allow_pickle=False) as artifact:
        ...
        indices = artifact["selection_indices"].astype(np.int32)
    ...
    for name, index in zip(names, indices):
        partition = partitions[int(index)]; truth = labels[mask]; predicted = partition[mask]
        sizes_full = np.bincount(partition, minlength=args.k); sizes_eval = np.bincount(predicted, minlength=args.k)
        record = {"lane": manifest["lane"], "arm": manifest["arm"], "config_id": manifest["config"]["config_id"],
                  "training_seed": manifest["training_seed"], "endpoint": str(name),
                  "candidate_id": str(candidate_ids[int(index)]), "partition_sha256": sha_array(partition),
                  "n_total": len(partition), "n_eval": int(mask.sum()), "k": args.k,
                  ...
                  "peak_rss_mib": manifest["peak_rss_mib"], **spatial_metrics(partition, graph)}
        if manifest["selections"][str(name)]["partition_sha256"] != record["partition_sha256"]:
...
def main() -> None:
    ...
    parser.add_argument("--authority", required=True); parser.add_argument("--k", type=int, required=True)
